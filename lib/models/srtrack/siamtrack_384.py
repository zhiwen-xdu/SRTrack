...
from lib.models.vipt.vit_siam_384 import vit_base_patch16_384_siam
...
class SIAMTrack(nn.Module):
    ...
    def forward(self, template: torch.Tensor,
        ...
        # 1) template：[B,6,128,128]，search：[B,6,256,256]
        # 2) Sequence Modeling: []
...
def build_siamtrack_384(cfg, training=True):
    ...
    if cfg.MODEL.BACKBONE.TYPE == 'vit_base_patch16_384_prompt':
        backbone = vit_base_patch16_384_siam(pretrained, drop_path_rate=cfg.TRAIN.DROP_PATH_RATE,
    ...
    if training:
        os_checkpoint = torch.load(cfg.MODEL.PRETRAIN_FILE, map_location="cpu")["net"]
        drop_checkpoint = torch.load("/home/czwos/Project/SIAMTrack/pretrained/DropTrack_k700_800E_alldata.pth.tar", map_location="cpu")["net"]
        for key, value in os_checkpoint.items():
            if "pos_embed_z" in key or "pos_embed_x" in key:
                model_state_dict[key] = drop_checkpoint[key]
                model_state_dict[key] = drop_checkpoint[key]
            elif "backbone.patch_embed." in key:
