...
class CoeSot(BaseVideoDataset):
    ...
    def __init__(self, root=None, split='train', image_loader=jpeg4py_loader_w_failsafe):
        ...
        if split is not None:
            ...
            if split == 'train':
                file_path = os.path.join(ltr_path, 'data_specs', 'coesot_train_split.txt')   # coesot_train_split.txt
            elif split == 'val':
                file_path = os.path.join(ltr_path, 'data_specs', 'coesot_val_split.txt')     # coesot_val_split.txt
            else:
