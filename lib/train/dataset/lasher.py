...
class LasHeR(BaseVideoDataset):
    ...
    def __init__(self, root=None, split='train', image_loader=jpeg4py_loader_w_failsafe):
        ...
        if split is not None:
            ...
            if split == 'train':
                file_path = os.path.join(ltr_path, 'data_specs', 'lasher_train_split.txt')
            elif split == 'val':
                file_path = os.path.join(ltr_path, 'data_specs', 'lasher_val_split.txt')
            else:
    ...
    def _get_data_path(self, seq_path, frame_id):
        ...
        img_path = os.path.join(seq_path, 'visible','{:04}.jpg'.format(frame_id+1))
        infrared_path = os.path.join(seq_path, 'infrared','{:04}.jpg'.format(frame_id+1))
        return img_path, infrared_path

    def _get_frame(self, seq_path, frame_id):
        ''' Return:img + infrared frame '''
        img_path, infrared_path = self._get_data_path(seq_path, frame_id)
        # [H,W,6]:[H,W,3]+[H,W,3]
        img = get_merge_frame(img_path, infrared_path)
        return img
