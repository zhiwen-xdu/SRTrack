...
class RewardActor04(BaseActor):
    ...
    def reward_function(self, sampled_boxes, gt_boxes, num_samples=255):
        ...
        gt_boxes_vec = box_xywh_to_xyxy(gt_boxes)[:, None, :].repeat((1, num_samples, 1)).view(-1, 4).clamp(min=0.0,max=1.0)   # [B*N,4]
        boxes = sampled_boxes.view(-1, num_samples, 4)                                                                     # [B,N,4]

        # 取N个Sampled Bboxs中GIoU+L1 Reward最高的Bbox
        giou = calculate_giou(sampled_boxes, gt_boxes_vec)                                                      # [B*N,]
        giou = giou.view(-1, num_samples)                                                                   # [B,N]
        l1 = l1_elementwise(sampled_boxes, gt_boxes_vec)                                                        # [B*N,4]
        ...
        best_boxes = (gt_boxes + best_boxes)/2                                                              # [B,4]
