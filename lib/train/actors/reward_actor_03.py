...
# 1) 随机采样，使用Critic扎到Reward最高的Bbox,把它当做GT BBox;
...
from lib.utils.focal_loss import calculate_focal
l1_elementwise = torch.nn.L1Loss(reduction='none')
l2_elementwise = torch.nn.MSELoss(reduction='none')


class RewardActor03(BaseActor):
    ...
    def sample_noise(self, boxes, num_samples=256, box_scale=0.03):
        """
        Sample from a gaussian noise the size and offset near the given boxes.
        Sample from a gaussian noise the amplitude near the given maps.
        :param boxes: (B,1,4)
        ...
        boxes_noise = torch.randn(batch_size, num_samples, 4).to(boxes.dtype).to(boxes.device) * box_scale   # [B,N,4]
        boxes_noise = torch.clamp(boxes_noise, min=-0.03, max=0.03)
        boxes_out = boxes.detach()
        boxes_out = boxes_out.repeat((1, num_samples, 1))                # [B,N,4]
        boxes_out = boxes_out + boxes_noise
    ...
    def reward_function(self, sampled_boxes, gt_boxes, num_samples=255):
        # sampled_boxes: [B*N,4]- [x,y,w,h], N表示采样的Bbox数;
        # gt_boxes: [B,4]- [x,y,w,h];
        # best_boxes: [B,4]- [x,y,x,y];
        # best_maps: [B,1,H,W]
        sampled_boxes = box_cxcywh_to_xyxy(sampled_boxes)                                                                  # [B*N,4]
        gt_boxes = box_xywh_to_xyxy(gt_boxes)[:, None, :].repeat((1, num_samples, 1)).view(-1, 4).clamp(min=0.0,max=1.0)   # [B*N,4]
        boxes = sampled_boxes.view(-1, num_samples, 4)                                                                     # [B,N,4]

        # 取N个Sampled Bboxs中GIoU+L1 Reward最高的Bbox
        giou = calculate_giou(sampled_boxes, gt_boxes)                                                      # [B*N,]
        giou = giou.view(-1, num_samples)                                                                   # [B,N]
        print("xxxx",sampled_boxes.shape,gt_boxes.shape,giou.shape)
        l1 = l1_elementwise(sampled_boxes, gt_boxes)                                                        # [B*N,4]
        l1 = l1.mean(dim=1)                                                                                 # [B*N,]
        l1 = - l1.view(-1, num_samples)                                                                     # [B,N]
        box_reward = giou + 2.5 * l1
        ...
        # gt_centers = (gt_boxes[:, 0:2] + gt_boxes[:, 2:4]) / 2                                              # [B*N,2]
        # sampled_centers = (sampled_boxes[:,0:2] + sampled_boxes[:,2:4])/2                                   # [B*N,2]
        # l2_location = l2_elementwise(sampled_centers,gt_centers)                                            # [B*N,2]
        # l2_location = l2_location.mean(dim=1)                                                               # [B*N,]
        # l2_location = l2_location.view(-1, num_samples)                                                     # [B,N]
        # center_index = torch.argmin(l2_location, dim=1)                                                     # [B,]
        # best_center_boxes = torch.cat([boxes[i, center_index[i], :][None, :] for i in range(len(center_index))], dim=0)  # [B,4]
        # best_center_boxes = box_xyxy_to_xywh(best_center_boxes)
        # # List
        # best_maps = generate_heatmap([best_center_boxes], self.cfg.DATA.SEARCH.SIZE,self.cfg.MODEL.BACKBONE.STRIDE)
        # best_maps = best_maps[-1].unsqueeze(1)                                                              # (B,1,H,W)
    ...
    def compute_losses(self, pred_dict, gt_dict, return_status=True):
        ...
        # [B*N,4]
        sampled_boxes = self.sample_noise(pred_boxes, num_samples=512)
        # [B,4]-[x,y,x,y], [B,1,H,W]
        best_boxes,best_maps = self.reward_function(sampled_boxes, gt_bbox, num_samples=512)
