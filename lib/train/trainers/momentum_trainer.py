...
class MomentumTrainer(BaseTrainer):
    def __init__(self, actor, loaders, optimizer, settings, lr_scheduler=None, use_amp=False):
        ...
        self.m = 0.05
        self.momentums_1 = [p.clone() * 0.0 for n, p in self.actor.net.backbone.named_parameters() if p.requires_grad]
        # self.momentums_2 = [p.clone() * 0.0 for n, p in self.actor.net.backbone.named_parameters() if p.requires_grad]
    ...
    def cycle_dataset(self, loader):
        ...
        for i, data in enumerate(loader, 1):
            ...
            # Momentum-based Fine-tune
            # θ(t-1)
            params_keep = [p.clone() * 0.0 for n, p in self.actor.net.backbone.named_parameters() if p.requires_grad]
            ...
            if loader.training:
                ...
                if not self.use_amp:
                    ...
                    self.optimizer.step()
                    self._momentum_update_1(params_keep)
                    # self._momentum_update_2(params_keep)
                    # self._momentum_update_3(params_keep)
                else:
    ...
    # @torch.no_grad()
    def _momentum_update_1(self, params_keep):
        """ Momentum update of the encoder """
        # param_keep: θ(t-1)
        # param_tuned: θ(t)
        # param_dalta: Δθ(t) = θ(t) - θ(t-1) / θ(t) = θ(t-1) + Δθ(t)
        # momentum update (momentum=v): v_new = m * v_old + Δθ(t)   momentum的会快速变大：0.0002->0.02
        # param update: θ(t)* = θ(t-1) + v_new = θ(t-1) + m * v_old + Δθ(t)
        params_tuned = [p for n, p in self.actor.net.backbone.named_parameters() if p.requires_grad]
        for momentum, param_tuned, param_keep in zip(self.momentums_1, params_tuned, params_keep):
            param_dalta = param_tuned.data - param_keep.data
            momentum.data = self.m * momentum.data + (1-self.m) * param_dalta
            param_tuned.data = param_keep.data + momentum.data


    # @torch.no_grad()
    # def _momentum_update_2(self, params_keep):
    #     """ Momentum update of the encoder """
    #     # param_keep: θ(t)
    #     # param_tuned: θ(t+1)
    #     # param_dalta: Δθ(t+1) = θ(t+1) - θ(t) / θ(t+1) = θ(t) + Δθ(t+1)
    #     # momentum update (momentum=Δθ(t-1)): Δθ(t) -> Δθ(t+1)  (先计算，再更新)
    #     # param update: θ(t+1)* = θ(t) + v_new = θ(t) + (1-m)*Δθ(t+1) + m*Δθ(t)
    #     params_tuned = [p for n, p in self.actor.net.backbone.named_parameters() if p.requires_grad]
    #     for momentum, param_tuned, param_keep in zip(self.momentums_2, params_tuned, params_keep):
    #         param_dalta = param_tuned.data - param_keep.data
    #         param_tuned.data = param_keep.data + param_dalta + self.m * (momentum.data - param_dalta)
    #         momentum.data = param_dalta


    @torch.no_grad()
    # def _momentum_update_3(self, params_keep):
    #     """ Momentum update of the encoder """
    #     # param_keep: θ(t)
    #     # param_tuned: θ(t+1)
    #     # param_dalta: Δθ(t+1) = θ(t+1) - θ(t) / θ(t+1) = θ(t) + Δθ(t+1)
    #     # momentum update (momentum=Δθ(t-1)): Δθ(t) -> Δθ(t+1)  (先计算，再更新)
    #     # param update: θ(t+1)* = θ(t) + v_new = θ(t) + Δθ(t+1) + m*(-Δθ(t)-Δθ(t+1))
    #     params_tuned = [p for n, p in self.actor.net.backbone.named_parameters() if p.requires_grad]
    #     for momentum, param_tuned, param_keep in zip(self.momentums_2, params_tuned, params_keep):
    #         param_dalta = param_tuned.data - param_keep.data
    #         param_tuned.data = param_keep.data + param_dalta + self.m * (-momentum.data - param_dalta)
